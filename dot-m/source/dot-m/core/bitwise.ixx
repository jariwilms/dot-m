...
export namespace dtm::bit
{
    ...
    auto constexpr set              (value_t value, index_t index, dtm::bool_t state) -> value_t
    {
    ...
    template<std::unsigned_integral value_t, std::unsigned_integral index_t>
    auto constexpr test             (value_t value, index_t index) -> dtm::bool_t
    {
    ...
    auto constexpr count_zero       (value_t value) -> value_t
    {
        ...
        else static_assert(dtm::false_, "invalid direction");
    }
    template<bit::direction_e direction_v, std::unsigned_integral value_t>
    ...
    template<std::unsigned_integral value_t>
    auto constexpr is_power_of_2    (value_t value) -> dtm::bool_t
    {
    ...
    auto constexpr shift            (value_t value, index_t index) -> value_t
    {
        ...
        else if constexpr (direction_v == bit::direction_e::right)
        {
            ...
            else static_assert(dtm::false_, "invalid shift");
        }
        else static_assert(dtm::false_, "invalid direction");
    }
    ...
    template<bit::direction_e direction_v, std::unsigned_integral value_t>
    auto constexpr rotate_carry     (value_t value, dtm::flag_t carry) -> std::tuple<value_t, dtm::flag_t>
    {
    ...
    auto constexpr pack             (from_t alpha, from_t beta) -> to_t
    {
        ...
        else static_assert(dtm::false_, "invalid endian");
    }
    template<bit::endian_e endian_v, std::unsigned_integral to_t, std::unsigned_integral from_t> requires (sizeof(from_t) > sizeof(to_t))
    ...
    template<bit::endian_e endian_v, dtm::size_t group_size_v, std::unsigned_integral value_t, std::unsigned_integral element_t = value_t> requires (group_size_v <= bit::length<value_t>())
    auto constexpr expand           (value_t value) -> std::array<element_t, (bit::length<value_t>() + group_size_v - 1u) / group_size_v>
    {
        ...
        for (auto index = dtm::size_t{ 0u }; index < value_count; ++index)
        {
    ...
    template<bit::endian_e endian_v, dtm::size_t group_size_v, std::unsigned_integral value_t, std::unsigned_integral element_t, std::size_t count_v> requires (group_size_v * count_v <= bit::length<value_t>())
    auto constexpr compress         (std::array<element_t, count_v> const& values) -> value_t
    {
        ...
        for (auto index = dtm::size_t{ 0u }; index < count_v; ++index)
        {
    ...
    auto constexpr test_carry       (value_t alpha, value_t beta, dtm::flag_t carry = dtm::false_) -> dtm::bool_t
    {
        ...
        else static_assert(dtm::false_, "invalid operation");
    }
    template<bit::operation_e operation_v, std::unsigned_integral value_t>
    auto constexpr test_half_carry  (value_t alpha, value_t beta, dtm::flag_t carry = dtm::false_) -> dtm::bool_t
    {
